#!/bin/sh
# run-16：SMOKE panel S3/S4/S5×2 轮（judge=deepseek-v3 真裁决验证）
cd /opt/build/pr/deploy
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#p4rt-20260917')
BK=/tmp/env-backup-p6r16-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P6-judge真裁决验证(S3S4S5x2)","mode":"L","datasetVersion":"eval-ds-1","panel":"SMOKE","idempotencyKey":"p6-judge-real-20260919"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
