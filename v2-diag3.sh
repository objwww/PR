...
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#v2diag3-20260917')
BK=/tmp/env-backup-v2diag3-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
echo '=== 控制端异常日志 ==='
docker logs deploy-control-app-1 --since 2m 2>&1 | grep -B1 -A6 'Exception\|ERROR' | head -30
cp "$BK" .env
