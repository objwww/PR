#!/bin/sh
echo '---kill hung worker---'
docker rm -f eval-worker-std >/dev/null 2>&1 || true
sleep 3
echo '---restart with fresh tag (orphan sweep will finalize honestly)---'
...
docker run -d --name eval-worker-std \
  ...
  --app.alert.eval.run-tag="p6g$(date +%H%M%S)" \
  --app.eval.launch.enabled=true \
  --app.eval.rounds=1 \
  > /dev/null
sleep 30
docker logs eval-worker-std 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | grep -E '评测执行注册表装载|Started|孤儿|Application run failed' | tail -4
echo '---batch e007de1c final---'
docker exec deploy-postgres-1 psql -U postgres -d pr_agent -At -c "select state||' '||coalesce(terminal_reason,'-') from eval_run where id::text like 'e007de1c%';"
echo '---disk---'
df -h / | tail -1
