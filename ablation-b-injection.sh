...
sleep 8
Q "select 'inbox_on='||state||'|err='||coalesce(last_error,'-') from alert_inbox where payload_raw::text like '%$FP1%' order by received_at desc limit 1;"
...
sleep 8
Q "select 'inbox_off='||state||'|err='||coalesce(last_error,'-') from alert_inbox where payload_raw::text like '%$FP2%' order by received_at desc limit 1;"
