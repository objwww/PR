...
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#p2r3-20260917')
BK=/tmp/env-backup-p2r3-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P2-回放E2E-3","mode":"L","datasetVersion":"eval-ds-1","idempotencyKey":"p2-replay-e2e-20260917-3"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
