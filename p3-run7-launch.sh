...
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#p3r7-20260917')
BK=/tmp/env-backup-p3r7-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P3-三维评分E2E-7","mode":"L","datasetVersion":"eval-ds-1","idempotencyKey":"p3-dims-e2e-20260917-7"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
