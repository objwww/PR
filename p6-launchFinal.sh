...
BK=/tmp/env-backup-p6gF-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P6-GATE最终验证(5簇x1轮UI修复后)","mode":"L","datasetVersion":"eval-ds-1","panel":"SMOKE","roundsPerScenario":1,"idempotencyKey":"p6-gate-final-ui-20260919"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
