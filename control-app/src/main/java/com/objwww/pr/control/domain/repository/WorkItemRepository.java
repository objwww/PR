...
/**
 * I17：租约/过期比较走 DB now()，签名不含应用时钟（INC-30/TB-07）。
 */
public interface WorkItemRepository {
    ...
    /**
     * 领取下一个可执行 WorkItem（READY/RETRY_WAIT 且 available_at &lt;= DB now()，
     * 按 priority DESC, available_at, created_at 排序，SKIP LOCKED）。
     * 原子写租约：lease_owner=owner、lease_until=DB now()+min(step.timeout_seconds, maxLeaseSeconds)、
     * lease_epoch+1、state=LEASED、attempt_count+1。无可领取项返回空。
     */
    Optional<WorkItem> claimNext(String owner, int maxLeaseSeconds);

    /**
     * 心跳续租：仅当租约仍属 (leaseOwner, leaseEpoch) 且 state=LEASED 时延长 lease_until
     * （lease_until = DB now() + leaseSeconds，新租约窗口由 DB 时钟计算）。
     * 0 行（返回 false）= 已被判死/重领，持有方必须停止干活。
     */
    boolean heartbeat(UUID id, String leaseOwner, long leaseEpoch, int leaseSeconds);

    /** 崩溃恢复扫描（CT-02/ST-08）：过期 LEASED 项（lease_until &lt; DB now()），按 lease_until 升序 */
    List<WorkItem> findExpiredLeases(int limit);

    /**
     * 回收过期租约：仅当 id 仍处 LEASED 且 lease_epoch 匹配且 lease_until &lt; DB now() 时
     * 置为 target（READY=预算未尽重领 / DEAD=耗尽），lease_epoch+1、清空 owner/until，
     * available_at=DB now()。0 行（返回 false）= 已被他人处理。
     */
    boolean reclaimExpiredLease(UUID id, long leaseEpoch, WorkItemState target);
    ...
    int cancelActiveByRunId(UUID reviewRunId);
}
