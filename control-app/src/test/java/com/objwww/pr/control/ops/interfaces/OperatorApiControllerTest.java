 ...
 * CasesView.vue 实际调用为准）：RBAC 正反、summary/list/detail 投影形状、
 * 并发认领 409 携最新投影、resolve 结构化 reason、审计断言。
 * 拆解验收："RBAC、并发认领、审计测试"。
 */
class OperatorApiControllerTest {
    ...
    void setUp() {
        mvc = MockMvcBuilders.standaloneSetup(
                        new OperatorApiController(query, service, "op-token-1"))
                .build();
    }
    ...
    void rbacRejectsMissingAndWrongBearer() throws Exception {
        mvc.perform(get("/api/cases/summary")).andExpect(status().isUnauthorized());
        mvc.perform(get("/api/cases/summary").header("Authorization", "Bearer wrong"))
    ...
    void summaryMatchesViewTabsShapeWithBucketCounts() throws Exception {
        ...
        mvc.perform(get("/api/cases/summary").header("Authorization", BEARER))
    ...
    void listSortsBySlaRiskWithOverdueFirst() throws Exception {
        ...
        mvc.perform(get("/api/cases").header("Authorization", BEARER))
                .andExpect(status().isOk())
    ...
    void listFiltersByStatusPriorityReasonCode() throws Exception {
        ...
        mvc.perform(get("/api/cases").param("status", "OPEN").header("Authorization", BEARER))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1))
                .andExpect(jsonPath("$[0].status").value("OPEN"));

        mvc.perform(get("/api/cases").param("priority", "P1").header("Authorization", BEARER))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1))
                .andExpect(jsonPath("$[0].priority").value("P1"));
    }
    ...
    void detailReturnsWorkspaceProjectionWithAuditTrail() throws Exception {
        ...
        mvc.perform(get("/api/cases/" + id).header("Authorization", BEARER))
                .andExpect(status().isOk())
    ...
    void detailUnknownCaseIs404() throws Exception {
        mvc.perform(get("/api/cases/" + UUID.randomUUID()).header("Authorization", BEARER))
    ...
    @Test
    void claimSetsOwnerAndReturnsCaseProjection() throws Exception {
        UUID id = seed("fp-claim", "P0", null, false, false);

        mvc.perform(post("/api/cases/" + id + "/claim").header("Authorization", BEARER)
    ...
    void resolveRejectsMissingReasonAndAcceptsStructuredShape() throws Exception {
        ...
        mvc.perform(post("/api/cases/" + id + "/resolve").header("Authorization", BEARER)
                        .header("X-Operator-Id", "operator-a")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void assignAcceptsSpecToOwnerAndViewAssignee() throws Exception {
        ...
        mvc.perform(post("/api/cases/" + id + "/assign").header("Authorization", BEARER)
                        .header("X-Operator-Id", "dispatcher")
                        .contentType(MediaType.APPLICATION_JSON)
