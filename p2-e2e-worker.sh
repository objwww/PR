#!/bin/sh
# P2 回放 E2E 第二段：eval worker（worker 模式）领取 LAUNCH 命令执行回放批
set -eu
cd /opt/build/pr

# 回放专用空注册表（scenarios: []——执行面=纯 case_version 回放案例）
...
SPRING_JSON=$(printf '{"app":{"alert":{"eval":{"registry-path":"file:/registry-replay-empty.yml","lexicon-path":"file:/eval/synonym-lexicon-v1.yml","dataset-version":"eval-ds-1","model":"glm-5","prompt-version":"am3-rca-v2","prompt-digest":"%s","tool-registry-digest":"%s","provider-fingerprint":"litellm:glm-5@dashscope","alert-rule-digest":"%s","grader-version":"grader-p2-replay-v1","webhook-url":"http://control-app:8080/webhooks/alertmanager","webhook-bearer":"%s"}},"eval":{"launch":{"enabled":true,"modes":"L"}}}}' "$PROMPT_DIGEST" "$TOOL_DIGEST" "$RULE_DIGEST" "$BEARER")
...
# 后台常驻 worker（领取并执行 eed0e10d 的 LAUNCH 命令；批完自动退出由外层 docker --rm 收容）
nohup docker run --rm --name eval-worker-p2replay \
  --entrypoint java \
  ...
  > /tmp/p2-worker.log 2>&1 &
echo "worker pid=$!"
sleep 25
echo '--- worker 启动日志 ---'
grep -E 'Started ControlApplication|评测执行注册表装载|ERROR|REJECTED' /tmp/p2-worker.log | tail -8
