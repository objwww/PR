...
| CL-06 累计记忆 | CL批执行者 | V101 schema_version/parent_memory_id/ofV2 真 revision+父链+跨轮反证并集保留→目标：反证跨轮可达、控制拒绝不混业务 ruled_out | WORKTREE 未提交 | V101 已应用 | ContextAssemblerTest/WorkingMemory 单测；195 树 PostgresWorkingMemoryIT 5/5 | A13-04 复核；B2 真机 run 1a5175cb 结构面 PASS（记忆行 3 父链连号+锚点回环+槽纯度+FULL 捕获；**2026-09-13 审查后证明力降级：v1 断言层四假绿面成立（verify 无退出码/父序反转静默/空父集 vacuous/phase7 未绑案/phase9 INNER JOIN 漏锚——docs/告警-BUGLOG与B2测试审查-统一收口技术方案-v1.md §4），PASS 数据本体幸存（memory-chain/checkpoint-anchor 独立定格）但验证器证明力待 v2 重建**；**2026-09-13 v2 真窗收官（S0 三件套 8d79901→b2-cl06v2-20260913/）：verify2 契约真机面成立，18 run/12 有效，结构面 8/8 全过（LAG 父链+NULL-safe 锚点+槽纯度+FULL 对账），RETRY2-VERDICT: INCONCLUSIVE（exit 3）=非空 witness 未取得** | **记忆结构=VERIFIED_TARGET；非空反证跨轮可达=IMPLEMENTED_NOT_VERIFIED（BA-142：结构性不可达，待生产面增强）** | **BA-142 定谳**：回执生产者 NativeInvestigationExecutor.submitDelegationReceipt 成功路径硬编码 counterRefs=空/missing=空，子任务=确定性单工具角色不读委派任务描述→counter_evidence_refs 非空无生产路径、open_gaps 仅 FAILED 子任务可达且不可引导；另 BA-141（PRIMARY 未驱动即 DEAD→native 未决兜底→run SUCCEEDED，全史 17 例）同窗立案。升级路径=回执结构化生产面（N 系规格线），非测试面 |
...
| OR-06 依赖故障有界 | B2 主会话 | RR21~24 执行器（rr2124-rr21.sh/rr2224-run.sh+受控故障 mock） | **RR21 5/5 PASS**（真表写拒/不假 ACK+四表零增量/接收器日志留痕/恢复重投单 run/幂等不重复）+**RR22 4/4 PASS**（429/慢首字节/慢体/RST：物理对拍 mock 击数==账本行数、有界≤6、零 fallback）+**RR24 3 PASS+Clock NOT_RUN**（badproto/TLS 拒握/DNS 不可解析：物理证据=DEFERRED durable-defer 记账；Clock 需专用 VM 归专用线）+**RR23 1/4 PASS**（LEMPTY 空结果语义与不可达可区分+logs.aggregate×6 全链可达+evidence 零伪造；LDEAD/LTRUNC/LSLOW=deepseek 逐 run 是否发出 logs 调用不可控（BA-120 族步预算），归 native 确定性路径或更大步预算模型——如实 NOT_VERIFIED）。迭代六雷立案 BA-144 全部修复关单；证据 rr21-20260913/、rr2224-20260913/ | **VERIFIED_TARGET（RR21/RR22/RR24 全量+RR23 空结果语义面）；RR23 fail 族待确定性触发** | RR23 残余归 RR25/26 native 路径或模型窗；CLOCK 归专用 VM 线 |
| OR-07 入口积压通知 | B2 主会话 | RR25~28 执行器（rr2528-run.sh/rr26-run.sh）；BA-126/127 立案（影子收口/Run 对账）归本卡+CL-02 | **RR25 九相位全 PASS**（首见铸/gen0；重复通知幂等键；同 episode 材料变化不换代；仅 severity 不触发重查；resolved 保持 generation；重复 resolved 幂等；乱序迟到不复活=旧 firing 不冒充新故障；resolved→firing generation+1 新 episode）+**RR27 PASS**（多物理请求一次副作用：同载荷×3 物理投递=1 条事件；fingerprint+payloadHash+startsAt 幂等键）+**RR26 4 PASS+AGE 落档**（基线/阶梯两档 11/11 全 202 持久接收；慢模型拖 worker 队列峰值深度=21 排空归零；p95=0.013/0.019s 时延样本计算；最老年龄 50s/225s 有界；材料序号化 RERUN 铸 run=11）+**RR28 PASS**（同 episode 乱序防御/新 episode generation+1 语义，随 RR25 相位实证）——零模型依赖设计（非白名单名 BUCKETED_HOLMES 零 run），notification_count=重复通知计数口径登记；证据 rr2528-20260913/ 130 文件 | **VERIFIED_TARGET（RR25/26/27/28 全量）** | OR-07 四案收官 |
