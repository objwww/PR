...
try {
    ...
    & "$auditJava/bin/javac.exe" -cp $auditCp -d docs/review-tools/build docs/review-tools/SafeClosedBoundaryAudit.java
    if ($LASTEXITCODE -ne 0) { throw 'javac failed' }
    & "$auditJava/bin/java.exe" -cp "docs/review-tools/build;$auditCp" SafeClosedBoundaryAudit
