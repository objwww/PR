...
while [ $valid -lt $N ] && [ $att -lt $MAXATT ]; do
  ...
  D="$(dirname "$D")"   # grep -l 命中的是 run-id.txt 文件本身，取其目录才是 run 目录
  V2="$(cat "$D/phase10-verify2.exit" 2>/dev/null || echo '?')"
  echo "attempt $att valid: drv=$DRV run=$RID verify2=$V2 dir=$D" | tee -a "$AGG"
  valid=$((valid+1))
  # 分类优先级（v2.1 勘误）：V2 已知时以验证器语义为准——driver 的 phase10 门
  # 设计上以 r7_fail（exit 1）编码 verify2=3（INCONCLUSIVE 不得整体 PASS），
  # 故 DRV=1 ∧ V2=3 是预期编码不是失败；V2=?（driver 死在 phase10 前）才是 FAIL。
  if [ "$V2" = "1" ] || [ "$V2" = "2" ] || [ "$V2" = "?" ]; then
    fail_seen=1
    echo "  -> FAIL（drv=$DRV verify2=$V2；末错行：$(grep -E 'FAIL' "$LOG" | tail -1 | cut -c1-140)）" | tee -a "$AGG"
  elif [ "$V2" = "3" ]; then
    echo "  -> INCONCLUSIVE（verify2=3：结构过/非空 witness 未取得——drv=$DRV 为 phase10 门预期编码）" | tee -a "$AGG"
  elif [ "$V2" = "0" ] && [ "$DRV" = "0" ]; then
    pass_seen=1
    echo "  -> PASS（verify2=0 结构+非空 witness 双过）" | tee -a "$AGG"
  else
    fail_seen=1
    echo "  -> FAIL（V2=0 但 drv=$DRV：phase10 后异常退出）" | tee -a "$AGG"
  fi
done
