...
if [ "${_nev:-0}" -ge 1 ]; then
    ...
    r7_log "phase5 PASS（直查全落 allowlist；证据 ${_nev} 行引用全锚定）"
else
    # 诚实未决分支（B2 门：UNKNOWN 诚实）——零证据必须留 final_claims+missing_information
    _fcl="$(r7_psql_ro R7_PG_URL "SELECT (final_claims IS NOT NULL AND jsonb_array_length(final_claims) >= 1
        AND final_missing_information IS NOT NULL AND jsonb_array_length(final_missing_information) >= 1)
        FROM rca_primary_checkpoint WHERE run_id='${RUNID}'" '-At')"
    [ "$_fcl" = "t" ] || r7_fail "phase5 证据恒零且非诚实未决（final_claims/missing_information 缺）"
    r7_log "phase5 PASS（诚实未决：零证据+缺口留痕）"
fi
...
# phase7 报告链（BA-134 口径：同 incident 非首跑=发布仲裁 loser 落档不发布，合法）
r7_log "phase7 报告链：report → publication（winner）或 loser（同 incident 已发布）"
r7_db_poll_ge "phase7 report 落行" 60 R7_PG_URL \
    "SELECT count(*) FROM rca_report WHERE run_id='${RUNID}'" 1
_pub="$(r7_psql_ro R7_PG_URL "SELECT count(*) FROM report_publication p JOIN rca_report rr ON p.report_id=rr.id
     WHERE rr.run_id='${RUNID}'" '-At')"
if [ "${_pub:-0}" -ge 1 ]; then
    ...
    r7_log "phase7 PASS（winner 面：publication+outbox 落行）"
else
    docker logs deploy-control-app-1 --since 30m 2>&1 \
      | grep -q 'report_publication_loser' \
      && r7_log "phase7 PASS（loser 面：同 incident 发布位已被更早报告占据——仲裁语义正确，outbox 不外发）" \
      || r7_fail "phase7 既非 winner 也无 loser 标记（发布链断）"
fi
...
# 父链不变量（委派流校准）：唯最低版次行可无父——委派获批先于首个 STEP 消耗版次
# （委派步不写记忆行），首行落 rev>=1 而非 0；其余行必须有父且父版次=子版次-1。
_chainbrk="$(r7_psql_ro R7_PG_URL "SELECT count(*) FROM rca_working_memory c
    ...
    WHERE c.run_id='${RUNID}'
      AND ((c.parent_memory_id IS NULL
            AND c.checkpoint_revision <> (SELECT min(checkpoint_revision)
                                          FROM rca_working_memory WHERE run_id='${RUNID}'))
        OR (c.parent_memory_id IS NOT NULL
            AND (p.id IS NULL OR p.checkpoint_revision <> c.checkpoint_revision - 1)))" '-At')"
[ "$_chainbrk" = "0" ] || r7_fail "phase9 父链断裂/跳号 $_chainbrk 处（首行须为无父最低版，其余父=前一版连号）"
# 锚点不变量（委派流校准）：final 提交也消耗版次（checkpoint 终版可 > 末记忆行版次），
# 真不变量=checkpoint 锚定本 run 最新记忆行且 digest 对账。
_anchor="$(r7_psql_ro R7_PG_URL "SELECT count(*) FROM rca_primary_checkpoint cp
    ...
    WHERE cp.run_id='${RUNID}'
      AND (m.checkpoint_revision <> (SELECT max(checkpoint_revision)
                                     FROM rca_working_memory WHERE run_id='${RUNID}')
           OR m.memory_digest <> cp.memory_digest)" '-At')"
[ "$_anchor" = "0" ] || r7_fail "phase9 检查点记忆锚点漂移 $_anchor 处（须锚定最新记忆行+digest 对账）"
