#!/bin/sh
# 第 13 轮部署：调查路由可视化（routing/overview）+ 详情页等待卡 + 三页美化（不轮换口令）
set -e
cd /opt/build/pr
export JAVA_HOME=/opt/jdk-21.0.12.1+1
export PATH="$JAVA_HOME/bin:$PATH"
cd /tmp && tr -d '\r' < p321-batch.tar.md5 | md5sum -c -
cd /opt/build/pr
tar xf /tmp/p321-batch.tar -C /opt/build/pr
echo '=== 1) 源码在场 ==='
grep -c 'routing/overview' control-app/src/main/java/com/objwww/pr/control/alert/interfaces/RoutingOverviewController.java
...
echo '=== 3) docker build（control-app+web） ==='
cd deploy
...
curl -s -b $J "http://127.0.0.1:8080/api/v1/routing/overview" | head -c 800; echo
echo '=== web chunk 复核 ==='
