#!/bin/sh
# 标准评测 worker @127（MIG-127，2026-09-21）——eval-worker 执行面迁 127（2C4G 评测机）
# 与 195 evalw.sh 的差异：DB/litellm/control-app/chaos-admin 全部改走 WireGuard 隧道面
# （195=10.250.250.1；chaos 经 chaos-wg-relay 中继 18099——eval-mgmt 为 internal 网不可直挂端口）
...
# RUN_TAG 不注入（2026-09-21 A/B 窗口实证教训）：静态 tag=worker 生命周期级，同 worker 第二批必撞
# uq_chaos_scenario 永存行（409 零注入全灭）；缺省走 BA-190 W1 正路=按批派生 "r<evalRunId>" 天然唯一

# MIG-127：全部跨机端点走隧道面；chaos-admin-url 指向 relay（18099）
SPRING_JSON=$(printf "{\"app\":{\"alert\":{\"eval\":{\"registry-path\":\"file:/eval/eval-scenarios.yml\",\"lexicon-path\":\"file:/eval/synonym-lexicon-v1.yml\",\"dataset-version\":\"eval-ds-1\",\"model\":\"deepseek-v3\",\"prompt-version\":\"am4-native-v5\",\"prompt-digest\":\"%s\",\"tool-registry-digest\":\"%s\",\"provider-fingerprint\":\"litellm:deepseek-v3@dashscope\",\"alert-rule-digest\":\"%s\",\"grader-version\":\"grader-am3-v1\",\"webhook-url\":\"http://10.250.250.1:8080/webhooks/alertmanager\",\"webhook-bearer\":\"%s\",\"prometheus-url\":\"http://10.250.250.1:9090\",\"arena-base-url\":\"http://10.250.250.1:8082\",\"chaos-admin-url\":\"http://10.250.250.1:18099\"}},\"eval\":{\"launch\":{\"enabled\":true,\"modes\":\"L\",\"max-rounds\":30},\"rounds\":2}}}" "$PROMPT_DIGEST" "$TOOL_DIGEST" "$RULE_DIGEST" "$BEARER")

docker rm -f eval-worker-std 2>/dev/null || true
# MIG-127 加固：--restart unless-stopped（127 重启/容器崩溃自愈）+json-file 日志轮转（不再 nohup 重定向——
# restart 后 attach 客户端不会重连，重定向会丢重启后日志；查日志用 docker logs eval-worker-std）
docker run -d --name eval-worker-std \
  --restart unless-stopped \
  --log-opt max-size=64m --log-opt max-file=2 \
  --entrypoint java \
  -v /opt/build/pr/deploy/alert/eval:/eval:ro \
  -e SPRING_DATASOURCE_URL=jdbc:postgresql://10.250.250.1:5432/pr_agent \
  ...
  -e APP_ALERT_EVAL_LITELLM_BASE_URL=http://10.250.250.1:4100 \
  -e APP_ALERT_EVAL_LITELLM_MASTER_KEY="$LITELLM_MK" \
  -e APP_ALERT_EVAL_JUDGE_BASE_URL=http://10.250.250.1:4100 \
...
sleep 25
docker logs eval-worker-std 2>&1 | grep -E "Started ControlApplication|注册表装载|ERROR|REJECTED" | tail -6
