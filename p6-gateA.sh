...
'alertname=ArenaPendingPaymentBacklog|service=order-arena|job=order-arena',
'alertname=ArenaPaymentOrderMismatch|service=order-arena|job=order-arena',
'alertname=ArenaDuplicatePayments|service=order-arena|job=order-arena') order by incident_key;"
echo '---any Arena FIRING---'
...
BK=/tmp/env-backup-p6gA-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P6-GATE战役A(5簇x1轮)","mode":"L","datasetVersion":"eval-ds-1","panel":"SMOKE","roundsPerScenario":1,"idempotencyKey":"p6-gate-campaign-a2-20260919"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
