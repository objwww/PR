...
BK=/tmp/env-backup-p6r23-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P6-FUP04锚点容差验证(S3S4S5x1)","mode":"L","datasetVersion":"eval-ds-1","panel":"SMOKE","roundsPerScenario":1,"idempotencyKey":"p6-fup04-verify-20260919"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
