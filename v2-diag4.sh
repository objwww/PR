...
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#v2diag4-20260917')
BK=/tmp/env-backup-v2diag4-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
echo '=== 真实异常堆栈 ==='
docker logs deploy-control-app-1 --since 3m 2>&1 | grep -A14 'threw exception' | head -40
cp "$BK" .env
