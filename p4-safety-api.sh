#!/bin/sh
# P4 安全面端点真验：凭据换入式登录（与 p4-run9.sh 同律），GET /api/eval/runs/{id}/safety
RUN_ID=$1
if [ -z "$RUN_ID" ]; then echo 'usage: p4safety.sh <runId>'; exit 2; fi
cd /opt/build/pr/deploy
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#p4rt-20260917')
BK=/tmp/env-backup-p4api-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
echo '---/safety---'
curl -s -b $J -H "X-XSRF-TOKEN: $T2" \
  "http://127.0.0.1:8080/api/eval/runs/$RUN_ID/safety" -w '\nhttp=%{http_code}\n'
cp "$BK" .env
