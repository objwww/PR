#!/bin/sh
# p12b 发批 B1：全数据集 ×2 轮（质量门双批之第一）
set -e
cd /opt/build/pr/deploy
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#p12b-gate-20260920')
BK=/tmp/env-backup-p12b-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P12b-质量门双批B1(5场景x2轮)","mode":"L","datasetVersion":"eval-ds-1","roundsPerScenario":2,"idempotencyKey":"p12b-gate-b1-20260920"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
echo 'env-restored'
sleep 8
