...
set -e
cd /opt/build/pr
echo '4a842d8f680dc675e927792e8b8c2910  /tmp/fup03-full.tar.gz' | md5sum -c -
tar xzf /tmp/fup03-full.tar.gz
...
echo '--- std worker 换新 run-tag 重建（新镜像含 FUP-03 + judge 重试）---'
JUDGE_BASE=$(grep '^OPENAI_COMPAT_BASE_URL=' .env | cut -d= -f2- | tr -d '\r"')
JUDGE_KEY=$(grep '^AGENT_MODEL_API_KEY=' .env | cut -d= -f2- | tr -d '\r"')
docker inspect eval-worker-std --format '{{range .Config.Env}}{{println .}}{{end}}' > /tmp/std4.env
NEWTAG="p6$(date +%H%M%S)"
if grep -q '"run-tag":"[^"]*"' /tmp/std4.env; then
  sed -i "s|\"run-tag\":\"[^\"]*\"|\"run-tag\":\"$NEWTAG\"|" /tmp/std4.env
else
  sed -i "s|\"app\":{\"alert\":{\"eval\":{|&\"run-tag\":\"$NEWTAG\",|" /tmp/std4.env
fi
if ! grep -q '"judge":' /tmp/std4.env; then
  sed -i "s|{\"app\":{\"alert\":{\"eval\":{|&\"judge\":{\"base-url\":\"$JUDGE_BASE\",\"api-key\":\"$JUDGE_KEY\",\"model\":\"deepseek-v3\"},|" /tmp/std4.env
fi
echo "run-tag -> $(grep -o '"run-tag":"[^"]*"' /tmp/std4.env)"
