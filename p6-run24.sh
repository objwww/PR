...
BK=/tmp/env-backup-p6r24-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P6-GATE战役5簇A(1轮)","mode":"L","datasetVersion":"eval-ds-1","panel":"SMOKE","roundsPerScenario":1,"idempotencyKey":"p6-gate-5cluster-a-20260919"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
