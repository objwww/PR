...
> 2026-09-11审查注：本文保留为原始调研，不作为直接执行规格。后续源码审查发现旧PR模型账本不能直接等同RCA/评测账本；告警、任务、评分状态需分开，监控与评测共享事实但分入口。详细纠偏及页面/版本演进/API/48例验收见[执行审查方案](../../docs/告警-评测中心与能力版本演进-审查及详细改造方案-v1.md)。国内五张图片另见[人工视觉复核更正](参考界面-国内告警系统.md)，不能沿用旧图片分析作为已验证证据。
