...
| T3 | run 级指标补齐：EvalQueryService 增 tool_error_rate/repeated_action_rate/evidence_checkpoint_rate/conclusion_grounded_rate/recovery_rate/hallucination_proxy_rate/constraint_compliance_rate/unsafe_block_rate/human_escalation_rate/acceptance_rate/P50/P95/run 级成本合计；EvalRunDetailView 补展示（前端等脏集） | 后端无冲突 | ◐ 2026-09-19 **后端出数面 ✅**：GET /api/eval/runs/{id}/process-metrics 上线——ProcessMetricsRow 单查询现算（V140 过程列 SUM+verdict 结构面+conclusion_grounded GROUNDED 计数+percentile_cont P50/P95+rca_tool_invocation 关联 FAILED 计数）+服务装配六率（结构通过/重复动作/工具错误/检查点覆盖/结论有据+分位，分母 0 → null 如实不冒充 0%）+default 方法测试桩零漂移；测试 65/65 绿（新增 3：比率装配/零分母 null/未知 run）；幻觉独立率与恢复率需 judge/轮次收敛定义（标注后续批次）；拦截率与接受率聚合在 /safety 与反馈面（前端折算即可）；**剩余半场**：EvalRunDetailView 过程面卡+mdZh 文案（视图已净，下轮即做） |
