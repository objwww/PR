...
| T5 | 六要素指标化：scorer 增 conclusion_six_parts（六段非空+confidence 在场）；eval_case_result 增列；run 级 six_parts_rate；主 prompt v8→v9 补 confidence 显式段 | 后端无冲突 | ◐ 2026-09-19 **前置+后端接线半场 ✅**（前置见上一注记；接线因 9381bc58 主会话提交使 scorer/config 转净而开窗）：V152 eval_case_six_parts 独立 insert-only 表（沿 V141 先例，**不改宽表**——避让脏测试文件+列语义独立；缺席=无行如实）+EvalCaseSixPartsSink/Postgres 实现（业务键幂等 DO NOTHING；level_presence 约束=把握布尔与档位同生共死）+scorer 伸缩构造器第五全形（旧签名委托 null——脏 SingleCaseScorerTest 零破坏）+recordSixParts fail-soft（pkg+rawText 双面检出）+EvalRunnerConfig 接线；测试 71/71 绿（新 7+既有回归 64）；**剩余半场**：run 级 six_parts_rate 聚合（reader/service/controller 已转净可动）+EvalRunDetailView 展示卡+zh.js 键（zh.js 仍脏，届时评估 diff 夹带面） |
| T6 | 提示词人话重写：judge rubric v2 + primary v9 按 §6 风格规范重写，工作台 diff 验证；发布走版本中心受控激活 | judge（HttpEvalReportJudge）与主 prompt 装配（AlertAm4Config）均在主会话面 | ◐ 2026-09-19 **草案半场 ✅**：两份草案落 deploy/alert/prompts/（新文件零冲突）——draft-judge-rubric-v2.md（v1 三题保留+新增 Q4 六要素完整度+人话 system，解析契约逐字不变，RUBRIC_VERSION 升 v2 纪律注记）+draft-primary-v9-写作要求增量.md（不改协议键三增量：把握显式短语=conclusion_six_parts 的 regex 检出锚/好坏示例成对/禁黑话三条；v8 实测 F1=0.9655 动得多不如动得准）；风格依据 Extra08+吴恩达已引用；**应用半场待可写窗口**（QUESTIONS/system 替换+工作台 diff→审批→激活→role_digest 对账）+跑批校准待充值 |
...
| T8 | 全工具复杂场景 S26：掉单+变更回归复合，走 logs+prometheus+change.diff+rca_history+runbook→建议 restart→R2 审批→执行→恢复验证；注册 L4 案例+演练脚本 | 无冲突 | ⬜ |
