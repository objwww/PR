...
| T6 | 提示词人话重写：judge rubric v2 + primary v9 按 §6 风格规范重写，工作台 diff 验证；发布走版本中心受控激活 | judge（HttpEvalReportJudge）与主 prompt 装配（AlertAm4Config）均在主会话面 | ◐ 2026-09-19 **judge 半场应用 ✅**（草案见前轮注记；HttpEvalReportJudge 转净开窗）：Q4 六要素完整度题落 QUESTIONS+人话 system（四题/不放水/逐题独立）+RUBRIC_VERSION 升 judge-rubric-v2（版本冻结纪律）+EvalReportJudgeTest 四题面全改（题序/题数/非布尔/围栏）；测试 73/73 绿；草案文件状态注记已更新；**剩余半场**：primary v9 写作增量应用（AlertAm4Config 仍脏）+工作台 diff 验证+跑批校准（待充值） |
| T7 | 子 agent 提示词修改/发布/diff 流程：新增草稿端点（POST /api/v1/prompt-workbench/drafts→diff→发布=release_asset 新版本+ConfigBundle 激活+role_digest 对账）；前端工作台编辑/发布按钮（等脏集） | 后端无冲突 | ◐ 2026-09-19 **草稿面后端 ✅**：V153 prompt_draft 草稿表（关键设计修正——草稿=提案文档**非** release_asset 行：role_digest 是 AgentProfile 全量摘要草稿期不可伪造，发布仍走既有受控激活，零绕行）+PromptDraftController（起草=base 快照冻结/列表/diff 基线面行数组直出/discard CAS 单向四端点，鉴权随 /api/v1 既有规则）+契约测试 3/3 绿（状态机冻结/只写草稿表零 release_asset 写入/无 apply/publish/promote 绕行端点）；**剩余半场**：前端工作台草稿 UI（PromptWorkbenchView 仍脏）+应用半场（发布=受控激活对接，需 AlertAm4Config 窗口） |
| T8 | 全工具复杂场景 S26：掉单+变更回归复合，走 logs+prometheus+change.diff+rca_history+runbook→建议 restart→R2 审批→执行→恢复验证；注册 L4 案例+演练脚本 | 无冲突 | ◐ 2026-09-19 **详设+注册+演练脚本 ✅**（详设/注册见前两轮注记；本轮 ma-drill-s26.sh 落 drills/）：A 段=chaos 全流程沿 s16 家法当前可跑（激活→流量→firing→变更面计数→off→恢复→对账，s26 命名隔离）；B 段=充值门断言面（审批五表 SQL 模板+process-metrics/six-parts/judge v2 四端点断言清单，列名以 V114/V119 为准跑前核对）；**落地批剩余**：approval 链存在性 reader（评分观测面）+充值后实跑 |
