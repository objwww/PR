...
| T8 | 全工具复杂场景 S26：掉单+变更回归复合，走 logs+prometheus+change.diff+rca_history+runbook→建议 restart→R2 审批→执行→恢复验证；注册 L4 案例+演练脚本 | 无冲突 | ◐ 2026-09-19 **详设+注册 ✅**（详设见前轮注记；本轮 registry v5→v6 落地注册）：S26 块入 scenarios（24 场景装载门 3/3 绿——F9 单族/L4/六检查点含审批链锚）；md_suite 20 条账不动（S26 为套件外 L4 高级场景）；**落地批剩余**：approval 链存在性 reader+演练脚本 ma-drill-s26+跑批（充值门） |
