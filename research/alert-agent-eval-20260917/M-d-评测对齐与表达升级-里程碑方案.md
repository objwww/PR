...
| T8 | 全工具复杂场景 S26：掉单+变更回归复合，走 logs+prometheus+change.diff+rca_history+runbook→建议 restart→R2 审批→执行→恢复验证；注册 L4 案例+演练脚本 | 无冲突 | ◐ 2026-09-19 **详设备料 ✅**：S26-全工具复合场景详设.md 落档（七要素/七工具调用链表/审批五表验收清单/判分口径/落地清单/风险注记）；关键判定修正——S26 **不依赖 M-b 复合激活**（F9 单族走既有驱动，变更面是真实账本佐证+归因诱导项），可跑性门槛大幅降低；落地批剩：registry v6 注册+approval 链 reader+演练脚本+跑批（充值门） |
