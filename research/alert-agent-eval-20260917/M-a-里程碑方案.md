...
| T8 | 真栈演练 A | S16→S17→S18→S19 逐个：激活→firing→恢复→resolved→SQL 对账，记录时间线 | 4 场景全链闭环 | ◐ **S16 ✅ 2026-09-18**：全链闭环（201 激活→3 单注入损伤[SQL 对账 mismatch=3]→Prometheus firing[ArenaPaymentOrderMismatch page, gauge=3]→off→修复 repaired=3→gauge=0→ALERTS resolved→RECOVERED 审计→会话 CLOSED；episode 台账留 PAYMENT_ORDER_MISMATCH×3 损伤史）。**演练连带修复五处真机缺陷**：①pay() attempt_no 按 kind 取号撞 uq_payment_attempt（库内 160 万 AUTH/0 CAPTURE——M2 起从未跑通的潜伏 bug）②V9 ck_chaos_fault_type 漏扩 F9~F18（激活 409 掩蔽）③V10 ck_inj_fault_type 漏扩（恢复收口审计被拒）④V12 ck_probe_finding_type 漏扩（episode 开户被拒→C-7 失明→告警永不 firing）⑤V11 探测面性能事故（百万行全表扫，一轮 4min→节奏退化分钟级）+ 探测查询加 30min 回看窗（lookback-seconds=1800）。S17~S19 待续 |
