...
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#p10-f1drain-20260920')
BK=/tmp/env-backup-p10-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P10-FUP04a-F1窗口内清偿验证","mode":"L","datasetVersion":"eval-ds-1","panel":"SMOKE","roundsPerScenario":1,"idempotencyKey":"p10-f1drain-verify-20260920"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
