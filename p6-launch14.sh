#!/bin/sh
# run-14 发起：panel=SMOKE + datasetVersion=eval-ds-1（真故障注入三件套）
cd /opt/build/pr/deploy
NEWHASH=$(/opt/jdk-21.0.12.1+1/bin/java -cp /tmp/ssc.jar /tmp/Gen.java 'Tmp#p4rt-20260917')
BK=/tmp/env-backup-p6r14-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P6-真端到端SMOKE(S3S4S5)","mode":"L","datasetVersion":"eval-ds-1","panel":"SMOKE","idempotencyKey":"p6-real-e2e-smoke-20260918"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
