...
BK=/tmp/env-backup-p4r10-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P4-红队注入探针v2","mode":"L","datasetVersion":"rt-v2","idempotencyKey":"p4-redteam-v2-e2e-20260917"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
...
echo '--- worker 认领 ---'
grep -E '领取 LAUNCH' /tmp/p4-worker.log | tail -2
