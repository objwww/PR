...
docker inspect eval-worker-std --format '{{range .Config.Env}}{{println .}}{{end}}' > /tmp/std9.env
if echo "$NEWTAG" | grep -q .; then :; fi
if grep -q '"run-tag":"p6' /tmp/std9.env; then
  sed -i "s|\"run-tag\":\"[^\"]*\"|\"run-tag\":\"$NEWTAG\"|" /tmp/std9.env
else
  sed -i 's|{"app":{"alert":{"eval":{|&"run-tag":"'"$NEWTAG"'",|' /tmp/std9.env
fi
grep -o '"run-tag":"[^"]*"' /tmp/std9.env
