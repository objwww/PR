#!/bin/sh
# p8b：修正 alert-rule-digest 空 value（p8-deploy 在 cd deploy 后用了相对路径算 RULE_DIGEST）
# 批次未发、旧 tag 未消费，直接重建 worker + 换新 tag
set -e
...
NEWTAG="p8b$(date +%H%M%S)"
echo "NEWTAG=$NEWTAG RULE_DIGEST=$RULE_DIGEST"
