...
BK=/tmp/env-backup-p6r17-$(date +%Y%m%dT%H%M%S)
cp .env "$BK"
...
curl -s -b $J -H "X-XSRF-TOKEN: $T2" -H 'Content-Type: application/json' \
  ...
  -d '{"displayName":"P6-judge真裁决重试(S3S4S5x2)","mode":"L","datasetVersion":"eval-ds-1","panel":"SMOKE","idempotencyKey":"p6-judge-real-20260919-b"}' \
  -w '\nhttp=%{http_code}\n'
cp "$BK" .env
